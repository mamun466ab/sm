@extends('super.index')
@section('content')
@section('title', 'School Admin Request')
                      ...
                      @if (count($scl_admin_reqs) > 0)
                          @foreach ($scl_admin_reqs as $scl_admin_req)
                              ...
                              <tr class="">
                                  ...
                                  <td><a class="btn btn-success" href="{{ url('/admin-approve') }}/{{ $scl_admin_req->id }}">Approve</a>|| <a class="btn btn-danger" href="{{ url('/admin-delete') }}/{{ $scl_admin_req->id }}">Delete</a></td>
                              </tr>
                         
                           @endforeach
                          @elseif(count($scl_admin_reqs) == 0)
                             <tr class="aler alert-danger">
                                 <td colspan="9" style="text-align: center;">There is no School Request now</td>
                             </tr>
                          @endif
